...
| `pair-check.mjs` | **组合强度诊断**：哪两个角色凑一块特别强。`npm run balance` 只量单体，量不出组合——用户能玩出来的「这俩太强了」在单体表上看不见 |
| `dup-check.mjs` | 「两边允许选同样角色会不会更好玩」的对照实验，另含最干净的先手测量（镜像对局） |
| `campaign-tune.mjs` | **战役曲线校准器**：逐关扫 `enemyMod.hp`，直接给出「该填什么」。只读不写——自动写回太容易在没人看的时候把曲线改坏 |
...
- **每方 3 人（战役 2 人）**，唯一定义在 `state.js` 的 `teamSizeFor(mode)`。
  ...
  **改人数会大幅改变 AoE 技能的强度**——3v3 让弓手「穿透箭」一度冲到 69.2%。
