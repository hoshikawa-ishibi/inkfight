...
// 打断的目标：只要不在免疫期就能打断（规则在 combat.js 的
...
function damageWorth(d, foe, skill) {
  ...
  if (d < ehp) return d;
  return ehp + KILL_BONUS - (skill.inkCost || 1) * (1 - ehp / d);
}
...
function healPriority(f, skill, topThreat, tw, incoming) {
  ...
  const doomed =
    incoming &&
    incoming.targetId === f.id &&
    (incoming.dmg || 0) >= f.hp + (f.shield || 0)
      ? 2.2
      : 1;
...
function healTarget(friends, skill, tw, incoming) {
  ...
  return pool.reduce((a, b) =>
    healPriority(a, skill, topThreat, tw, incoming) >=
    healPriority(b, skill, topThreat, tw, incoming)
      ? a
      : b,
  );
}
...
function buffTarget(friends, skill, tw) {
  const fresh = friends.filter(
    (f) => !f.buffs.some((b) => b.type === skill.buffType),
  );
...
//
// 共享回合由 ink-ai 在整队候选里统筹演员和技能；本入口保留给兼容调用。
//
...
export function pickActor(team, foes, scene, opts = {}) {
  ...
  const best = (u) => {
    ...
    return Math.max(
      ...usable.map((sk) => scoreSkill(u, sk, live, alive, scene, opts)),
    );
  };
...
export function scoreSkill(u, s, foes, friends, scene, opts = {}) {
  ...
  const avgDefMul =
    1 -
    foes.reduce((n, f) => n + f.def, 0) /
      foes.length /
      (foes.reduce((n, f) => n + f.def, 0) / foes.length + 50);
  // 暴击是确定的了（蓄能条），所以评分也必须把它算进来——否则 AI 不会
  // 「留着大招砸在必暴的那一刀上」，而那正是任务 2b 想创造的决策。
  // 注意倍率要按**这个技能自己的** crit 加成算（暗影突袭自带 +40）。
  const dmgOf = (p, sk) =>
    atk * (p || 1) * sceneMul * avgDefMul * (sk && willCrit(u, sk) ? 1.5 : 1);
  ...
  const threatOnAlly =
    threat && threat.targetId && threat.targetId !== u.id ? threatDmg : 0;
  ...
  const preempt = (foe, d) =>
    threat && foe && threat.unitId === foe.id && d >= foe.hp + (foe.shield || 0)
      ? threatDmg
      : 0;
  ...
  // 胜率反而 +11.3」，因为省下的资源拿去放毒刃更划算，而毒刃那
  ...
  const chargeWorth = (sk) => {
    ...
    const best = Math.max(
      1,
      ...u.skills.filter((k) => k.power).map((k) => k.power),
    );
  ...
  const riders = (sk, foe, d) => {
    ...
    if (sk.debuff === "defDown")
      v += atk * bestPower(u) * avgDefMul * 0.2 * (sk.debuffDur || 0) * 0.6;
    // 腐化层只对带「腐化侵蚀」被动的角色有额外价值（每层每次攻击 +8）
    if (sk.corrupt && u.passive?.effect === "corruptBonus")
      v += sk.corrupt * CORRUPT_BONUS_PER_STACK;
    return v;
  };
  ...
  const allyDanger = others.length
    ? Math.min(...others.map((f) => f.hp / f.maxHp))
    : 1;
  ...
  const tempo =
    tempoW > 0 && dmgOptions.length
      ? Math.max(
          ...dmgOptions.map((k) => atk * k.power * sceneMul * avgDefMul),
        ) * tempoW
      : 0;
  ...
  switch (s.type) {
    ...
    case "stun": {
      ...
      const fullLost = works
        ? threat && threat.unitId === cand.id
          ? Math.max(threatDmg, threatOf(cand))
          : threatOf(cand)
        : 0;
      const controlWorth = fullLost * (1 - INTERRUPT_OUTPUT_MULTIPLIER);
      // 带 power 的打断技能自带一次伤害，打不断时它就退化成一个普通伤害技能
      const d = dmgOf(s.power || 0, s);
      return (
        damageWorth(d, cand, s) +
        riders(s, cand, d) +
        controlWorth +
        preempt(cand, d)
      );
    ...
    case "plague": {
      ...
      const corruptWorth =
        u.passive?.effect === "corruptBonus"
          ? (s.corrupt || 0) * foes.length * CORRUPT_BONUS_PER_STACK
          : 0;
    ...
    case "healAll": {
      ...
      const usable = friends.reduce(
        (n2, f) => n2 + Math.min(s.healAmt || 0, f.maxHp - f.hp),
        0,
      );
    ...
    case "shieldAll": {
      ...
      const worth = friends.reduce(
        (n2, f) => n2 + (s.shieldAmt || 0) * (f.shield > 0 ? 0.4 : 0.85),
        0,
      );
    ...
    case "cleanse": {
      ...
      const bad = friends.reduce(
        (n, f) => n + f.debuffs.length + (f.disrupted || f.stunned ? 1 : 0),
        0,
      );
    ...
    case "selfBuff": {
      ...
      const perTurn =
        s.buffType === "berserk"
          ? (s.selfDmg ?? BUFF_DEFAULTS.berserkSelfDmg)
          : 0;
    ...
    case "shield": {
      ...
      const guess =
        worth * (hpFrac < 0.5 ? 1.3 : 1.0) + protect * 22 * taunting - tempo;
    ...
    case "taunt": {
      ...
      const guess =
        allyRisk * (hpFrac > 0.5 ? 1.2 : 0.5) + hit - (s.power ? 0 : tempo);
    ...
    case "dodge": {
      ...
      return (
        (foes.reduce((n, f) => n + getEffectiveAtk(f), 0) / foes.length) *
          (hpFrac < 0.4 ? 1.4 : 0.7) -
        tempo
      );
    }
    ...
    case "revive": {
      // 不屈：血越少越该开
      const guess =
        (hpFrac < 0.35 ? (s.hpRestore || 0) * 1.5 : (s.hpRestore || 0) * 0.25) -
        tempo;
      // 已知那一击正好能打死自己：这就是保命符，价值等于「不死」本身
      if (threatOnMe >= u.hp + (u.shield || 0))
        return (s.hpRestore || 0) + threatOnMe * 0.5;
      return guess;
    }

    default:
      return (s.inkCost || 1) * 0.5;
  }
}
...
export function pickTarget(actor, skill, enemies, allies, opts = {}) {
  ...
  switch (skill.type) {
    ...
    case "cleanse": {
      ...
      const afflicted = friends.filter(
        (f) => f.debuffs.length > 0 || f.disrupted || f.stunned,
      );
      ...
      return (
        pool
          ...
          .sort(
            (a, b) =>
              b.debuffs.length +
              (b.disrupted || b.stunned ? 1 : 0) -
              (a.debuffs.length + (a.disrupted || a.stunned ? 1 : 0)),
          )[0] || null
      );
    }
    case "buff":
