...
import {
  canInkAct, inkActionCost, previewInkSkill, INK_RULES
...
function followupPossible(turn, allies, chosenActor, spent){
  ...
  return allies.some(u => u.alive && u !== chosenActor && !turn.acted.includes(u.id) &&
    u.skills.some(s => inkActionCost({ ...turn, remaining:left, chain:[...turn.chain, {}] }, u, s) <= left));
}
  ...
  for(const actor of allies){
    ...
    for(const skill of actor.skills || []){
      ...
      const raw = scoreSkill(actor, transformed, foes, allies, scene, {
        tempo:0.55, teamwork:1, ctx:makeTeamContext()
      });
...
export function chooseInkAction(turn, allies, foes, scene, {random=()=>Math.random(), noise=0}={}){
  ...
  for(const actor of liveAllies){
    for(const skill of actor.skills || []){
      ...
      const target = pickTarget(actor, transformed, liveFoes, liveAllies, {tempo:0.55, teamwork:1, ctx});
      const raw = scoreSkill(actor, transformed, liveFoes, liveAllies, scene, {tempo:0.55, teamwork:1, ctx});
      const predicted = target ? previewDmg(actor, transformed, scene) : null;
      const kill = predicted != null && predicted >= target.hp + (target.shield || 0) ? 34 : 0;
      const completes = cost === turn.remaining ? 7 : 0;
      const followup = followupPossible(turn, liveAllies, actor, cost) ? 4 : -3;
      const lookahead = followupValue(turn, liveAllies, liveFoes, scene, actor, cost) * 0.12;
      const setup = ['buff','buffAll','selfBuff','cleanse'].includes(skill.type) && followup > 0 ? 5 : 0;
      const flow = turn.relics.includes('flow') && turn.chain.length < 2 && cost === 1 ? 5 : 0;
      const economy = raw / cost + raw * 0.16 + lookahead + kill + completes + followup + setup + flow;
      const jitter = noise ? (random() * 2 - 1) * noise : 0;
      candidates.push({ actor, skill, target, value:economy + jitter });
    }
  }
  if(!candidates.length) return null;
  candidates.sort((a,b) => b.value - a.value || inkActionCost(turn,a.actor,a.skill) - inkActionCost(turn,b.actor,b.skill));

  // 留下的墨也是真实选择：当前最好动作还不如把墨化盾，就提前收手。
  if(turn.chain.length > 0 && candidates[0].value < shieldValue(turn, liveAllies)) return null;
