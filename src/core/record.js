...
export const MODE_LABEL = { ai:'人机', pvp:'双人', spectate:'观战', campaign:'战役' };
...
export function mvpOf(units){
  ...
  return units.reduce((best, u) => mvpScore(u) > mvpScore(best) ? u : best, units[0]);
}

// ── 构造一条记录 ───────────────────────────────────────────
// 入参就是 battle.js 结算时手上有的东西，不做额外查询。
export function makeRecord({
  mode, difficulty, scene, rounds, winner, side,
  stats, units, at, id, backfilled, stage,
}){
  ...
  return normalizeRecord({
    ...
    at: at || Date.now(),
    mode, diff: difficulty || null, scene: scene && scene.id ? scene.id : scene,
    rounds, winner, side: side == null ? null : side,
    stage: stage || null,
...
export function newRecordId(at){
  ...
  const r = Math.floor(Math.random() * 0x10000).toString(36).padStart(4, '0');
  return `r${t}${r}`;
}
...
export function normalizeRecord(r){
  ...
  return {
    ...
    units: (Array.isArray(r.units) ? r.units : []).map(u => ({
      ...
      player: int(u.player),
      dmg: int(u.dmg), heal: int(u.heal), kills: int(u.kills),
    })),
    maxHit: { dmg: int(r.maxHit && r.maxHit.dmg), name: String((r.maxHit && r.maxHit.name) || '') },
    ...(r.backfilled ? { backfilled: true } : {}),
  };
}

function sideTotals(t){
  return { dmg: int(t && t.dmg), heal: int(t && t.heal), kills: int(t && t.kills) };
}
...
export function auditRecord(rec, now = Date.now()){
  ...
  if(r.side !== null && r.side !== 1 && r.side !== 2) bad('玩家所在方不是 1 / 2 / 无');
  ...
  if(!(r.rounds >= 1 && r.rounds <= MAX_ROUNDS)) bad(`回合数 ${r.rounds} 不合理`);
  ...
  sides.forEach((side, i) => {
    if(side.length < 1 || side.length > MAX_TEAM) bad(`${i + 1} 方有 ${side.length} 人，不在 1~${MAX_TEAM} 的范围内`);
    side.forEach(u => {
  ...
  if(r.units.some(u => u.player !== 1 && u.player !== 2)) bad('有单位不属于任何一方');
  ...
  [1, 2].forEach(p => {
    ...
    ['dmg', 'heal', 'kills'].forEach(k => {
      ...
      if(sum !== tot[k]) bad(`${p} 方的${{dmg:'伤害',heal:'治疗',kills:'击杀'}[k]}分项之和 ${sum} 对不上总计 ${tot[k]}`);
    });
  });

  // 最高单次伤害：打出它的人必须真实存在，且这一击不可能超过他一整局的总伤害。
  if(r.maxHit.dmg < 0 || r.maxHit.dmg > MAX_SINGLE_HIT) bad(`最高单次伤害 ${r.maxHit.dmg} 不合理`);
  if(r.maxHit.dmg > 0){
    // 两边可能选到同一个角色（重名），所以取同名单位里总伤害最高的那个。
    const same = r.units.filter(u => u.name === r.maxHit.name);
    if(!same.length) bad(`最高单次伤害记在「${r.maxHit.name}」名下，但场上没有这个单位`);
    else if(r.maxHit.dmg > Math.max(...same.map(u => u.dmg)))
      bad(`「${r.maxHit.name}」的最高单次伤害 ${r.maxHit.dmg} 超过了他一整局的总伤害`);
...
export function summarize(records){
  ...
  list.forEach(r => {
    ...
    const seen = new Set();
    mine.forEach(u => {
      const c = byChar[u.charId] = byChar[u.charId] || { id:u.charId, n:0, w:0, dmg:0, heal:0, kills:0 };
      if(!seen.has(u.charId)){ seen.add(u.charId); c.n++; if(won) c.w++; }
      c.dmg += u.dmg; c.heal += u.heal; c.kills += u.kills;
    });
  });
  ...
  rated.forEach(r => {
    if(outcomeOf(r) === 'win'){ streak++; bestStreak = Math.max(bestStreak, streak); }
    else streak = 0;
  });
  current = streak;
  ...
  list.forEach(r => {
    if(r.side == null) return;
    r.units.filter(u => u.player === r.side).forEach(u => {
      if(!bestGame || u.dmg > bestGame.dmg) bestGame = { dmg:u.dmg, name:u.name, charId:u.charId, at:r.at, id:r.id };
    });
  });
  ...
  return {
    ...
    bestStreak, currentStreak: current,
    bestGame,
    avgRounds: list.length ? list.reduce((a, r) => a + r.rounds, 0) / list.length : 0,
