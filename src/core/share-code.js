...
import { normalizeRecord, auditRecord, RULESETS } from "./record.js";
import {
  utf8Bytes,
  bytesToUtf8,
  hmacSha256,
...
const RULESET_ORDER = [RULESETS.legacy, RULESETS.ink];
...
function packRecord(r) {
  ...
  return [
    r.v,
    r.id,
    Math.round(r.at / 1000),
    idxOf(MODE_ORDER, r.mode),
    idxOf(DIFF_ORDER, r.diff),
    idxOf(SCENE_ORDER, r.scene),
    r.rounds,
    r.winner,
    r.side == null ? 0 : r.side,
    r.stage == null ? 0 : r.stage,
    r.p1.dmg,
    r.p1.heal,
    r.p1.kills,
    r.p2.dmg,
    ...
    units,
    r.backfilled ? 1 : 0,
    Math.max(0, RULESET_ORDER.indexOf(r.ruleset)),
  ];
}

function unpackRecord(a) {
  if (!Array.isArray(a) || a.length < 20)
    throw new RangeError("记录字段数不对");
  const [
    v,
    id,
    ts,
  ...
  if(ruleset!==undefined && ![0,1].includes(ruleset))throw new RangeError('未知战绩规则版本');
  ...
  return normalizeRecord({
    ...
    ruleset: RULESET_ORDER[ruleset] || RULESETS.legacy,
...
const CODE_RE = new RegExp(
  `${SHARE_PREFIX}\\.([A-Za-z0-9_-]+)\\.([A-Za-z0-9_-]+)`,
);
...
export function decodeShare(text, now = Date.now()) {
  ...
  if (!sigOk && !parsedOk)
    return fail(
      "signature",
      "签名对不上，内容也解不出来——这串码被改过，或者复制时断了一截。",
    );
  if (!parsedOk)
  ...
  if (fv > SHARE_FORMAT_VERSION)
    return fail(
      "version",
      `这串分享码来自更新的版本（格式 v${fv}），本作只认得 v${SHARE_FORMAT_VERSION}。`,
    );
...
export function shareFileName(profile) {
  const safe =
    String((profile && profile.name) || "无名").replace(/[^\w一-龥-]/g, "") ||
    "无名";
  const d = new Date();
