...
export function initPresentation() {
  ...
  const backdrop = new URL(
    SCENES.find((s) => s.id === "spring").art.image,
    document.baseURI,
  ).href;
  ...
  document.getElementById("title-roster-count").textContent =
    `${CHARACTERS.length} 位墨境行者`;
  document.getElementById("quick-battle-note").textContent =
    `远征 / 人机 / 双人 / 观战，全部使用同一套共享墨规则`;
...
function selectHero(c) {
  ...
  document.getElementById("title-hero-skills").textContent = c.skills
    .map((s) => s.name)
    .join(" / ");
  document.getElementById("btn-quick-battle").textContent =
    `带${c.name}出战　↗`;
  document
    .querySelectorAll("[data-hero]")
    .forEach((b) =>
      b.setAttribute("aria-pressed", String(b.dataset.hero === c.id)),
    );
}
...
export function showSkillCue(actor, skill) {
  ...
  who.textContent = `玩家${actor.player} · ${actor.name}`;
  const name = document.createElement("strong");
  name.textContent = skill.name;
  copy.append(who, name);
