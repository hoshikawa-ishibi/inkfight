...
import {
  summarize, mvpOf, outcomeOf, normalizeRecord,
  sceneName, charName, MODE_LABEL,
} from '../core/record.js';
import { DIFF_LABEL } from '../core/combat.js';
import {
  encodeShare, decodeShare, shareFileText, shareFileName, SHARE_TRUST_NOTE,
} from '../core/share-code.js';
import {
  listRecords, saveRecord, deleteRecord, clearRecords,
  getProfile, setProfileName, displayName,
  listFriends, mergeFriend, deleteFriend, RECORD_LIMIT,
} from '../game/save.js';
...
function modeText(r){
  ...
  if(r.mode === 'ai') return `${base} · ${DIFF_LABEL[r.diff] || r.diff || '?'}`;
...
const teamNames = (r, p) => r.units.filter(u => u.player === p).map(u => u.name).join(' ');
...
export function initRecordsScreen(){
  ...
  shell.querySelectorAll('.records-tab').forEach(b => {
    ...
    b.dataset.bound = '1';
    b.addEventListener('click', () => { playSfx('click'); tab = b.dataset.tab; render(); });
  });
...
function bind(id, fn){
  ...
  el.dataset.bound = '1';
  el.addEventListener('click', () => { playSfx('click'); fn(); });
}
...
function render(){
  ...
  if(exportBtn){
    ...
    exportBtn.textContent = mine.length ? `📤 分享我的战绩（${mine.length}）` : '📤 分享我的战绩';
  }
  if(tab === 'mine') renderMine(body, mine);
  else if(tab === 'career') renderCareer(body, mine);
...
function renderMine(body, records){
  ...
  body.querySelectorAll('.rec-row').forEach(el => el.addEventListener('click', () => {
    playSfx('hover'); selectedId = el.dataset.id; render();
  }));
  renderDetail(records.find(r => r.id === selectedId), true);
}
...
function recordRow(r){
  return `<button class="rec-row${r.id === selectedId ? ' selected' : ''}" data-id="${esc(r.id)}">
    ...
    <div class="rec-row-foot">${esc(sceneName(r.scene))} · ${r.rounds} 回合${
      r.backfilled ? ' <span class="rec-tag-backfill">补录</span>' : ''}</div>
  </button>`;
}
...
function renderDetail(r, own, host){
  ...
  if(!el) return;
  if(!r){ el.innerHTML = '<div class="records-empty">选一局看看。</div>'; return; }
  const mvp = mvpOf(r.units);
  const rows = p => r.units.filter(u => u.player === p).map(u =>
  ...
  el.innerHTML = `
    ...
    ${r.backfilled ? `<div class="rec-backfill-note">这一局是<b>手工补录</b>的——战绩功能上线前打的，
  ...
  if(share) share.addEventListener('click', () => { playSfx('click'); openShareDialog([r], '这一局'); });
  const del = el.querySelector('[data-act="del"]');
  if(del) del.addEventListener('click', () => {
    playSfx('click');
    confirmBox('删除这一局？', '删掉之后就找不回来了。', () => {
      deleteRecord(r.id); selectedId = null; render();
    });
  });
}
...
function renderCareer(body, records){
  ...
  if(!s.total){
    body.innerHTML = '<div class="records-empty"><b>还没有数据。</b><p>打几局再来看。</p></div>';
    return;
  }
  const diffRows = Object.entries(s.byDiff).map(([k, v]) =>
    `<div class="rec-bar-row">
     ...
     </div>`).join('') || '<p class="records-note">还没有分难度的对局。</p>';

  const charRows = s.chars.slice(0, 16).map(c =>
    `<div class="rec-char-row">
       <b>${esc(charName(c.id))}</b>
       <span>${c.n} 局</span>
       <span>胜率 ${c.n ? (c.w / c.n * 100).toFixed(0) : 0}%</span>
       <span>累计伤害 ${c.dmg}</span>
     </div>`).join('') || '<p class="records-note">还没有可统计的出场。</p>';
  ...
  body.innerHTML = `
    <div class="records-career">
      ...
      ${s.rated < s.total ? `<p class="records-note">另有 ${s.total - s.rated} 局没有胜负归属（观战、双人），
        计入总场次但不计胜率。</p>` : ''}
      ...
      ${s.bestGame ? `<h4 class="rec-section">我打出过的单场最高伤害</h4>
  ...
  if(clear) clear.addEventListener('click', () => {
    playSfx('click');
    confirmBox('清空全部战绩？', `${s.total} 局记录会被删除，包括补录的那几局，且无法恢复。
      想留个底的话，先「分享我的战绩」把分享码存下来。`, () => {
      clearRecords(); selectedId = null; render();
    });
  });
}
...
function renderFriends(body){
  ...
  const s = summarize(f.records);
  body.innerHTML = `
    <div class="records-split">
      <div class="records-list">
        ${friends.map(x => `<button class="rec-row${x.pid === selectedPid ? ' selected' : ''}" data-pid="${esc(x.pid)}">
          <div class="rec-row-top"><b>${esc(displayName(x))}</b>
            <span class="rec-when">${x.records.length} 局</span></div>
      ...
      <div class="records-detail">
        ...
        <div class="rec-friend-games">${f.records.map(r => `
  ...
  body.querySelectorAll('[data-pid]').forEach(el => el.addEventListener('click', () => {
    playSfx('hover'); selectedPid = el.dataset.pid; render();
  }));
  body.querySelectorAll('[data-rid]').forEach(el => el.addEventListener('click', () => {
    playSfx('hover');
    renderDetail(f.records.find(r => r.id === el.dataset.rid), false,
      document.getElementById('records-friend-detail'));
    document.getElementById('records-friend-detail').scrollIntoView({ behavior:'smooth', block:'nearest' });
  }));
  const del = document.getElementById('btn-friend-del');
  if(del) del.addEventListener('click', () => {
    playSfx('click');
    confirmBox('移除这位好友的战绩？', '只是从你这台设备上删掉，他自己的战绩不受影响。', () => {
      deleteFriend(f.pid); selectedPid = null; render();
    });
  });
}
...
export function openShareDialog(records, what){
  ...
  const profile = getProfile();
  const code = encodeShare({ name: displayName(profile), pid: profile.pid }, list);
  const mask = openModal(`
    ...
    <div class="rec-modal-status" id="share-status"></div>`, true);

  const status = mask.querySelector('#share-status');
  const say = t => { status.textContent = t; };

  mask.querySelector('#share-code').addEventListener('focus', e => e.target.select());
  mask.querySelector('#share-copy').addEventListener('click', async () => {
    playSfx('click');
    say(await copyText(code) ? '已复制，粘给朋友就行。' : '复制没成功——上面的框里全选手动复制吧。');
  });
  ...
  if(navigator.share){
    ...
    btn.addEventListener('click', async () => {
      playSfx('click');
      try {
        await navigator.share({ title:'墨境之战 · 我的战绩', text: shareFileText(profile, list, code) });
        say('分享面板已打开。');
      } catch { say('分享取消了。'); }
    });
  }
  mask.querySelector('#share-close').addEventListener('click', () => { playSfx('click'); dismiss(mask); });
}
...
export function openImportDialog(){
  const mask = openModal(`
    ...
    <div class="rec-import-result" id="import-result"></div>`, true);
  ...
  mask.querySelector('#import-file').addEventListener('click', () => {
    playSfx('click');
    pickTextFile(text => { area.value = text; check(); });
  });
  mask.querySelector('#import-paste').addEventListener('click', async () => {
    playSfx('click');
    try { area.value = await navigator.clipboard.readText(); check(); }
    catch { out.innerHTML = '<p class="rec-bad">浏览器不让我读剪贴板，请手动 Ctrl+V 粘到上面的框里。</p>'; }
  });
  mask.querySelector('#import-go').addEventListener('click', () => { playSfx('click'); check(true); });
  mask.querySelector('#import-close').addEventListener('click', () => { playSfx('click'); dismiss(mask); });
  area.addEventListener('input', () => { out.innerHTML = ''; });
  ...
  function check(commit){
    ...
    const badge = res.signature === 'valid'
    ...
    out.innerHTML = `
      ...
      ${res.profile ? `<p class="rec-modal-note">来自 <b>${esc(res.profile.name)}</b>，
        共 ${res.records.length} 场，导出于 ${when(res.profile.exportedAt)}。</p>` : ''}`;

    if(!res.ok){
      out.innerHTML += '<p class="rec-modal-note">校验没过的战绩不会被存进好友榜。</p>';
      return;
    }
    ...
    if(isSelf){
      ...
      let added = 0;
      res.records.forEach(x => { if(!have.has(x.rec.id)){ saveRecord(x.rec); added++; } });
    ...
    const r = mergeFriend(res.profile, res.records.map(x => x.rec));
    out.innerHTML += r.ok
...
function confirmBox(title, text, onYes){
  ...
  mask.querySelector('[data-act="no"]').addEventListener('click', () => { playSfx('click'); dismiss(mask); });
  mask.querySelector('[data-act="yes"]').addEventListener('click', () => {
    playSfx('click'); dismiss(mask); onYes();
  });
}

async function copyText(text){
  try { await navigator.clipboard.writeText(text); return true; }
  catch { /* 非安全上下文或用户拒绝，退回老办法 */ }
  try {
...
export function commitRecord(rec){
  try { return saveRecord(rec); }
  catch { return null; }   // 战绩存不下不该把结算流程带崩
}
