...
describe("集火：整队盯住同一个目标，不为一两点血差反复横跳", () => {
  ...
  test("敌方奶妈不会因为攻击力低就被无视", () => {
    ...
    for (const other of ["guardian", "archer", "berserker"]) {
      assert.equal(
        focusFoe([priest, createUnit(other, 2, 1)], null, 1),
        priest,
        `面对 ${other} 时应当先杀牧师`,
      );
    }
...
describe("不重复增益：同一个 buff 不往同一个人身上叠第二次", () => {
  ...
  test("优先加给还没带同类 buff 的队友", () => {
    ...
    assert.equal(
      pickTarget(priest, bless, foe(), friends, { teamwork: 1 }),
      mage,
  ...
  test("全队都带着同类 buff 时，再加一次就不值得出手了", () => {
    ...
    const before = scoreSkill(fresh[0], bless, foe(), fresh, VOID, {
      teamwork: 1,
    });
    const after = scoreSkill(buffed[0], bless, foe(), buffed, VOID, {
      teamwork: 1,
    });
    assert.ok(
      before > 0,
      `没人带 buff 时祝福应当划算（实得 ${before.toFixed(1)}）`,
    );
    assert.ok(
      after < 0,
      `全队已有 atkUp 时不该再祝福（实得 ${after.toFixed(1)}）`,
    );
  });
  ...
  test("简单难度察觉不到重复（teamwork=0 时照样往上叠）", () => {
    const buffed = [createUnit("priest", 1, 0), createUnit("mage", 1, 1)];
    buffed.forEach((f) => f.buffs.push({ type: "atkUp", dur: 3, value: 0.5 }));
    const dumb = scoreSkill(buffed[0], bless, foe(), buffed, VOID, {
      teamwork: 0,
    });
    const smart = scoreSkill(buffed[0], bless, foe(), buffed, VOID, {
      teamwork: 1,
    });
...
describe("保护残血队友：坦克在队友濒危时顶上去", () => {
  function scoreWith(skill, allyHpFrac, teamwork) {
    ...
    return scoreSkill(guardian, skill, foes, [guardian, mage], VOID, {
      teamwork,
    });
  }
  ...
  for (const [name, type] of [
    ...
    test(`${name} 的保护加成对简单难度无效`, () => {
      assert.equal(
        scoreWith(skill, 0.2, 0),
        scoreWith(skill, 1.0, 0),
  ...
  test("嘲讽在身时加盾更值（等于在替队友挡刀）", () => {
    ...
    const ally = () => {
      const m = createUnit("mage", 1, 1);
      m.hp = 18;
      return m;
    };
    assert.ok(
      scoreSkill(taunting, shield, foes, [taunting, ally()], VOID, {
        teamwork: 1,
      }) >
...
describe("治疗优先级：同等缺血时先救输出更高的队友", () => {
  ...
  test("溢出的治疗不算收益（48 点治疗只能补 48 点缺口）", () => {
    ...
    const barelyHurt = scoreSkill(priest, heal, foes, [priest, mage], VOID, {
      teamwork: 1,
    });
    mage.hp = mage.maxHp - 48;
    const properlyHurt = scoreSkill(priest, heal, foes, [priest, mage], VOID, {
      teamwork: 1,
    });
    assert.ok(
      properlyHurt > barelyHurt,
      "只掉 5 点血时不该急着放 48 点的大治疗",
    );
  });
});

describe("过量杀伤：不用大招去收最后一丝血", () => {
  test("残血目标面前，1墨基础技比3墨终结技划算", () => {
    const mine = [createUnit("swordsman", 1, 0)],
      foes = [createUnit("guardian", 2, 0)];
    foes[0].hp = 5;
    foes[0].dodge = 0;
    const basic = mine[0].skills[0];
    const big = mine[0].skills.reduce((a, b) =>
      b.inkCost > a.inkCost ? b : a,
    );
    assert.ok(
      scoreSkill(mine[0], basic, foes, mine, VOID) >
        scoreSkill(mine[0], big, foes, mine, VOID),
    );
  });
  ...
  test("护盾会让「能补刀」的判断落空（45 点盾挡在前面就不算残血）", () => {
    ...
    const basic = mine[0].skills[0];
    assert.ok(
      scoreSkill(mine[0], basic, bare, mine, VOID) >
...
describe("AI 选中的技能一定拿得到合法目标（防 battle.js 空指针）", () => {
  ...
  for (const [level, fn] of [
    ["简单", aiEasy],
    ["普通", aiNormal],
    ["困难", aiHard],
  ]) {
    test(`${level}难度：全员满血时，选中的技能都有合法目标`, () => {
  ...
  test("全员满血时治疗是负分（不会被困难难度选中）", () => {
    ...
    const foes = [createUnit("guardian", 2, 0)];
    assert.ok(
      scoreSkill(mine[0], heal, foes, mine, VOID, { teamwork: 1 }) < 0,
      "没人受伤时治疗应当算成亏本买卖",
    );
  });
});
