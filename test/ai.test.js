...
describe("ai.js 可在 Node 中直接运行（不依赖 DOM / gameState）", () => {
  ...
  test("所有难度只会选择原生墨耗技能", () => {
    const mine = team(["mage"], 1),
      foes = team(["guardian"], 2);
    for (const [name, fn] of levels) {
      const d = fn(mine[0], foes, mine, VOID);
      assert.ok(
        d?.skill.inkCost >= 1 && d.skill.inkCost <= 3,
        `${name}选择了非法墨耗技能`,
      );
    }
  });
    ...
    for (const c of CHARACTERS) {
      ...
      for (let i = 0; i < 40; i++) {
        ...
        if (needsEnemyTarget(d.skill)) {
          assert.ok(
            d.target,
            `${c.name}·${d.skill.name} 需要敌方目标却返回了 null`,
          );
...
describe("三档难度确实存在梯度（合并评分后手感不能被抹平）", () => {
  ...
  function quality(fn, n = 300) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (d?.skill) {
        sum += scoreSkill(mine[0], d.skill, foes, mine, scene);
        count++;
      }
    }
  ...
  test("简单难度比困难明显更常抡普攻（保留新手手感）", () => {
    ...
    const easy = basicRate(aiEasy),
      hard = basicRate(aiHard);
    assert.ok(
      easy > hard + 0.15,
...
describe("AI 的失误要看得见，而且只能出现在低难度", () => {
  ...
  test("简单难度经常犹豫，玩家看得出对面在浪费机会", () => {
    ...
    assert.ok(
      r > 0.15,
      `简单难度的犹豫率只有 ${(r * 100).toFixed(1)}%，玩家感觉不到`,
    );
  });
