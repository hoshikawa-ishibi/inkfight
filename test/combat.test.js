...
describe("calcDamage", () => {
  ...
  test("暴击 1.5 倍伤害", () => {
    ...
    const r = withRandom(0, () =>
      calcDamage(actor, target, { power: 1 }, null),
    );
  ...
  test("dot/defDown/selfHeal 技能字段会附加对应状态", () => {
    ...
    assert.ok(
      target.debuffs.some(
        (d) => d.type === "poison" && d.value === 5 && d.dur === 3,
      ),
    );
  ...
  test("atkUp1 buff 生效一次后被消耗", () => {
    const actor = makeUnit({
      atk: 20,
      buffs: [{ type: "atkUp1", dur: 1, value: 0.5 }],
    });
    const target = makeUnit();
    const r = calcDamage(actor, target, { power: 1 }, null);
  ...
  test("造成伤害时触发 onDamageDealt/onTakeDamage/onCrit 被动，事件带在返回值里", () => {
    ...
    assert.equal(actor.critMeter, 12);
    const chargeEvent = r.passiveEvents.find(
      (e) => e.event.effect === "critCharge",
    );
    assert.ok(chargeEvent, "应包含 critCharge 被动事件");
  });
});

// 打断现在是确定性的：同样局面给出同样结果，不掷骰。
...
describe("暴击蓄能条：确定性，且期望值不变", () => {
  test("攒满 100 才暴击，攒的速度就是暴击率", () => {
    ...
    for (let i = 0; i < 8; i++) {
      hits.push(
        calcDamage(actor, makeUnit({ def: 0, dodge: 0 }), { power: 1 }).isCrit,
      );
    }
    // 每次 +25，第 4、8 次攒满
  ...
  test("技能自带暴击加成算进蓄能速度", () => {
    ...
    // 10 + 40 = 50/次 → 第 2 次必暴
    assert.equal(
      calcDamage(actor, makeUnit({ def: 0, dodge: 0 }), { power: 1, crit: 40 })
        .isCrit,
      false,
    );
    assert.equal(
  ...
  test("结果与 Math.random 无关", () => {
    ...
    assert.equal(
      withRandom(0, () =>
        calcDamage(a, makeUnit({ def: 0, dodge: 0 }), { power: 1 }),
      ).isCrit,
      true,
    );
    assert.equal(
      withRandom(0.99, () =>
        calcDamage(b, makeUnit({ def: 0, dodge: 0 }), { power: 1 }),
      ).isCrit,
      true,
    );
  });
  ...
  test("长期期望伤害与旧的概率模型一致（20% 暴击 = 平均 1.1 倍）", () => {
    ...
    for (let i = 0; i < N; i++) {
      total += calcDamage(
        actor,
        makeUnit({ def: 0, dodge: 0, hp: 1e9, maxHp: 1e9 }),
        { power: 1 },
      ).dmg;
    }
    const avg = total / N;
...
describe("被动闪避：改成确定性减伤，期望值不变", () => {
  test("10% 闪避 = 恒定少受 10% 伤害，而不是 10% 概率完全免疫", () => {
    ...
    const b = calcDamage(
      actor,
      makeUnit({ def: 0, dodge: 10, hp: 1e9, maxHp: 1e9 }),
      { power: 1 },
    ).dmg;
  ...
  test("闪避不再让攻击落空（dodged 恒为 false）", () => {
    ...
    for (let i = 0; i < 50; i++) {
      assert.equal(
        calcDamage(actor, makeUnit({ dodge: 90 }), { power: 1 }).dodged,
        false,
      );
    }
  });
...
describe("calcStun：确定性打断", () => {
  const SKILL = { type: "stun" };

  test("存活且不在免疫期 → 必定打断（连跑 50 次结果完全一致）", () => {
    for (let i = 0; i < 50; i++) {
      const target = makeUnit();
  ...
  test("结果与 Math.random 无关（这正是本次改动的全部意义）", () => {
    const a = makeUnit(),
      b = makeUnit();
    assert.equal(
      withRandom(0, () => calcStun(makeUnit(), a, SKILL)).success,
      true,
    );
    assert.equal(
      withRandom(0.99, () => calcStun(makeUnit(), b, SKILL)).success,
      true,
    );
  });
  ...
  test("canInterrupt 和 calcStun 判断一致（评分与执行必须同口径）", () => {
    ...
    const immune = makeUnit({ interruptImmune: 1 });
    assert.equal(canInterrupt(ready), true);
    assert.equal(canInterrupt(immune), false);
    assert.equal(calcStun(makeUnit(), ready, SKILL).success, true);
    assert.equal(calcStun(makeUnit(), immune, SKILL).success, false);
  });
  ...
  test("扰乱按「产出什么」折算：持续伤害和层数爆发照样打折，净化不打折", () => {
    ...
    const poison = consumeInterruptedSkill(dis(), {
      type: "damage",
      power: 1.5,
      dot: 8,
      dotDur: 3,
    ...
    const burst = consumeInterruptedSkill(dis(), {
      type: "corruptBurst",
      dmgPerStack: 9,
    }).skill;
    assert.equal(
    ...
    const heal = consumeInterruptedSkill(dis(), {
      type: "heal",
      healAmt: 42,
    }).skill;
    assert.equal(heal.healAmt, Math.round(42 * M));

    const cleanse = consumeInterruptedSkill(dis(), {
      type: "cleanse",
      healAmt: 20,
    }).skill;
    assert.equal(
...
describe("triggerPassive - 原生共享墨被动效果", () => {
  test("critCharge：暴击后充值暴击蓄能", () => {
    const u = makeUnit({
      critMeter: 10,
      passive: {
        name: "残心",
    ...
    assert.equal(u.critMeter, 22);
    assert.equal(ev.effect, "critCharge");
  });

  test("selfShield：受伤后获得护盾", () => {
    const u = makeUnit({
      shield: 3,
      passive: {
        name: "自动机括",
    ...
    const ev = triggerPassive("onTakeDamage", u, { dmg: 10 });
    assert.equal(u.shield, 10);
    assert.equal(ev.effect, "selfShield");
  });

  test("selfHeal：治疗按缺失生命截断，满血时不产生事件", () => {
    const p = {
      name: "回春",
      trigger: "onTurnStart",
      effect: "selfHeal",
    ...
    const hurt = makeUnit({ hp: 95, passive: p });
    const ev = triggerPassive("onTurnStart", hurt);
    assert.equal(hurt.hp, 100);
    assert.equal(ev.value, 5);
    assert.equal(triggerPassive("onTurnStart", makeUnit({ passive: p })), null);
  });
  ...
  test("corruptBonus：按腐化层数造成额外伤害，可致命", () => {
    ...
    assert.equal(
      triggerPassive("onDamageDealt", warlock, { target: noCorrupt }),
      null,
      "无腐化层不触发",
    );

    // 每层的伤害读 combat.js 的常量，不写死——它是个平衡旋钮，会被调
    const corrupted = makeUnit({
      hp: 2 * CORRUPT_BONUS_PER_STACK,
      debuffs: [{ type: "corrupt", dur: 99, value: 2 }],
    });
...
describe("processStartOfTurn", () => {
  test("中毒扣血 + 狂暴扣血 + buff/debuff 到期衰减", () => {
    const u = makeUnit({
      ...
      buffs: [
        { type: "berserk", dur: 1, value: 0.4 },
        { type: "atkUp", dur: 2, value: 0.2 },
      ],
    });
    ...
    assert.equal(
      u.buffs.length,
      1,
      "berserk(dur1) 到期移除，atkUp(dur2) 仍保留一回合",
    );
...
describe("腐化机制：applyCorrupt / applyPlague / applyCorruptBurst", () => {
  ...
  test("applyPlague 同时施加腐化与中毒", () => {
    ...
    assert.ok(
      target.debuffs.some((d) => d.type === "corrupt" && d.value === 2),
    );
    assert.ok(
      target.debuffs.some(
        (d) => d.type === "poison" && d.value === 7 && d.dur === 3,
      ),
    );
  });
  ...
  test("applyCorruptBurst：按层数结算伤害并清空腐化，无腐化的目标不计入命中", () => {
    ...
    const stacked = makeUnit({
      id: "a",
      debuffs: [{ type: "corrupt", dur: 99, value: 3 }],
    });
    const clean = makeUnit({ id: "b", debuffs: [] });
    const { hits, totalDmg } = applyCorruptBurst(actor, [stacked, clean], {
      dmgPerStack: 22,
    });
    ...
    assert.equal(
      stacked.debuffs.some((d) => d.type === "corrupt"),
      false,
      "结算后腐化层应清空",
    );
  });
  ...
  test("applyCorruptBurst 可致命并在结果里报告", () => {
    ...
    const target = makeUnit({
      hp: 10,
      debuffs: [{ type: "corrupt", dur: 99, value: 5 }],
    });
...
describe("回归测试：术士（warlock）在无头模拟中不再静默失效", () => {
  ...
  test("腐化爆发按之前施加的层数真正结算伤害（旧 sim.js 里是死代码分支）", () => {
    const corruptBurst = warlockDef.skills.find(
      (s) => s.type === "corruptBurst",
    );
    const warlock = createUnit("warlock", 1, 0);
    const target = createUnit("swordsman", 2, 0);
    target.debuffs.push({ type: "corrupt", dur: 99, value: 3 });
    const { hits, totalDmg } = applyCorruptBurst(
      warlock,
      [target],
      corruptBurst,
    );
...
describe("createUnit / getEffectiveAtk / previewDmg", () => {
  ...
  test("getEffectiveAtk 叠加 atkUp/berserk buff", () => {
    const u = makeUnit({
      atk: 20,
      buffs: [{ type: "atkUp", dur: 1, value: 0.5 }],
    });
...
describe("applyStageMod（战役关卡的属性旋钮）", () => {
  ...
  test("atk / def 是乘在原属性上的倍率", () => {
    const u = makeUnit({ atk: 20, def: 10 });
    applyStageMod(u, { atk: 1.1, def: 0.9 });
  ...
  test("只写了一个字段时，其它属性一律不动", () => {
    const u = makeUnit({ atk: 20, def: 10, maxHp: 100 });
...
describe("createUnit 的 override（战役身份 / 墨皇）", () => {
  ...
  test("只给 name 时，只有名字变，属性一律照旧", () => {
    const u = createUnit("berserker", 2, 0, {
      id: "berserker",
      name: "荒野狂徒·赤牙",
    });
  ...
  test("墨皇：改属性但沿用术士的技能组和被动", () => {
    ...
    assert.equal(
      u.skills.length,
      CHARACTERS.find((c) => c.id === "warlock").skills.length,
    );
  });
