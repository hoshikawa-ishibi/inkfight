...
function snap(u) {
  return {
    ...
    shield: u.shield,
    alive: u.alive,
    critMeter: u.critMeter,
    disrupted: u.disrupted,
    stunned: u.stunned,
    dodging: u.dodging,
    undying: u.undying,
    atk: u.atk,
    crit: u.crit,
    passiveStacks: u.passiveStacks,
    buffs: u.buffs,
    debuffs: u.debuffs,
  };
}
...
// 直接扫共享执行器源码里的 case 标签。用源码而不是维护一份手写清单，
// 是因为手写清单必然和代码漂移——这个项目已经因为「同一份知识两份实现」
// 出过三次 bug 了。
function handledTypes() {
  const src = fs.readFileSync(
    new URL("../src/core/skill-executor.js", import.meta.url),
    "utf8",
  );
  return [
    ...new Set(
      [...src.matchAll(/case\s+["']([a-zA-Z]+)["']\s*:/g)].map((m) => m[1]),
    ),
  ];
}
...
function seed(charId, skill) {
  ...
  [actor, ally, foe, foe2].forEach((u) => {
    u.dodge = 0;
  });

  // 施法者留出 HP 余量，让吸血和自疗看得出来。
...
// executeSkill 会在 switch 之前支付生命代价，
    ...
    c.skills.forEach((skill) => {
      test(`${c.name} · ${skill.name}（${skill.type}）执行后确实改变了战场状态`, () => {
        ...
        const before = JSON.stringify(
          units.map((u) => (u === s.actor ? payCost(u, skill) : snap(u))),
        );
...
describe("哨兵：别让上面那组悄悄变成空跑", () => {
  ...
  test("data.js 用到的每一种技能类型，共享执行器的 switch 都有对应 case", () => {
    const used = new Set(
      CHARACTERS.flatMap((c) => c.skills.map((s) => s.type)),
    );
    ...
    assert.deepEqual(
      missing,
      [],
      `data.js 里有技能用了这些类型，但共享执行器的 switch 没接：${missing.join(", ")}`,
    );
  });
  ...
  test("switch 里没有任何技能都触达不到的死 case", () => {
    ...
    assert.deepEqual(
      orphan,
      [],
      `共享执行器的 switch 有 case 但没有任何技能用：${orphan.join(", ")}。` +
        `要么给它配个技能，要么删掉——留着只会让人以为游戏有这个机制`,
    );
  });

  test("局面确实造对了：不满血、有 debuff、有腐化层", () => {
    // 这几条是上面所有断言的前提。前提塌了，整组测试会变成「什么都没测出来」
    // 却依旧全绿——和它要防的那个 bug 是同一个物种。
    const s = seed("priest", CHARACTERS[4].skills[1]);
    assert.ok(
      s.actor.hp < s.actor.maxHp,
      "施法者该是残血的，否则看不出治疗/吸血",
    );
    assert.ok(
      s.ally.hp < s.ally.maxHp - 40,
      "队友血要缺得够多，否则治疗量被 clamp 掉",
    );
    assert.ok(
      s.ally.debuffs.length > 0 && s.ally.disrupted,
      "队友身上要有东西可净化",
    );
    assert.ok(
      s.foe.debuffs.some((d) => d.type === "corrupt"),
      "敌人要有腐化层可引爆",
    );
    assert.equal(
      s.foe.dodge,
      0,
      "闪避必须清零，否则钉死的随机数会让敌人躲光一切",
    );
  });
});
