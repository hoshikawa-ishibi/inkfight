...
import { INK_AI_NOISE } from "../src/core/ink-turn.js";
...
const LEVELS = ["easy", "normal", "hard", "nightmare"].map((k) => ({
  ...
  noise: INK_AI_NOISE[k],
}));

// 玩家永远是 p1（游戏里也是玩家先手，这份先手优势要保留在测量里）
function run(pNoise, oNoise) {
  let wins = 0,
    rounds = 0;
  for (let i = 0; i < N; i++) {
    const ids = shuffle(CHARACTERS.map((c) => c.id));
    const scene = SCENES[Math.floor(Math.random() * SCENES.length)];
    const r = simOneBattle(
      ids.slice(0, N_TEAM),
      ids.slice(N_TEAM, N_TEAM * 2),
      scene,
      { p1Noise: pNoise, p2Noise: oNoise },
    );
    if (r.winner === 1) wins++;
    rounds += r.rounds;
  }
...
console.log(
  "先手在这个战斗节奏下值约 10 个百分点，拿 50% 当基准会把每档都误判成偏难。\n",
);
...
for (const p of PLAYERS) {
  const fair = run(p.noise, p.noise).wr;
  const cells = LEVELS.map((l) => run(p.noise, l.noise));
